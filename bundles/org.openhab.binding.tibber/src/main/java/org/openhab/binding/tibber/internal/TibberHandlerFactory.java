 ...
 * @author Bernd Weymann - Use Storage for history data
 */
@NonNullByDefault
...
public class TibberHandlerFactory extends BaseThingHandlerFactory {
    ...
    public TibberHandlerFactory(final @Reference HttpClientFactory httpFactory, final @Reference CronScheduler cron,
        ...
        Utils.init(FrameworkUtil.getBundle(this.getClass()));
        // TODO remove this when the binding is stable
        Storage<String> store = storageService.getStorage(TibberHistory.class.getName());
    ...
    protected @Nullable ThingHandler createHandler(Thing thing) {
        ...
        if (thingTypeUID.equals(TIBBER_THING_TYPE)) {
            return new TibberHandler(thing, httpFactory.getCommonHttpClient(), cron, timeZoneProvider, storageService);
        } else {
            return null;
        }
    }
