...
class EntsoETests {
        ...
        public void testHandler() {
                ...
                ThingImpl thing = new ThingImpl(THING_TYPE_DAY_AHEAD_PRICES, "4711");
                ...
                // httpMock.when(() -> HttpUtil.executeUrl("GET", anyString(),
                // 30000)).thenReturn(response);
                // httpMock.when(HttpUtil::executeUrl("GET", anyString(),
                // 30000)).thenReturn(response);
                // }
                // ;
                // httpMock.when(() -> HttpUtil.executeUrl("GET", anyString(),
                // 30000)).thenReturn(response);
                // when(httpMock.executeUrl("GET", anyString(), 30000)).thenReturn(response);
                ...
                when(requestMock.timeout(30000,
                                TimeUnit.MILLISECONDS)).thenReturn(requestMock);
