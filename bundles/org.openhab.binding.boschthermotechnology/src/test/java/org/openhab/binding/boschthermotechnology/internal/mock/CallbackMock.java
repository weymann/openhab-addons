 ...
 */
package org.openhab.binding.boschthermotechnology.internal.mock;
...
/**
 * The {@link CallbackMock} is the {@link ThingHandlerCallback} used by every handler unit test in
 * this binding, copied from the project's shared {@code rules/test-fixtures/CallbackMock.java} and
 * adjusted only in its package declaration - see {@code rules/testing-rules.md}.
 *
...
public class CallbackMock implements ThingHandlerCallback {
    ...
    public void waitForStatus(ThingStatus expectedStatus) {
        ...
        // if method is exited without reaching the expected status e.g. through timeout, fail
        ...
        synchronized (stateMap) {
            ...
            while (state == null && Duration.between(start, Instant.now()).getSeconds() < STATUS_DURATION_TIMEOUT_SEC) {
                try {
                    ...
                    fail("Interrupted waiting for channel state");
                }
            }
            return state;
        }
