...
/**
 * A {@link ThingHandlerCallback} test double: captures channel state updates ({@link #stateMap}), trigger events
 * ({@link #triggerMap}), and thing status, with blocking helpers to wait for a given status without resorting to
 * {@code Thread.sleep(...)} (see {@code rules/testing-rules.md}).
 *
...
public class CallbackMock implements ThingHandlerCallback {
    ...
    public void stateUpdated(ChannelUID channelUID, State state) {
        synchronized (stateMap) {
            stateMap.put(channelUID.getId(), state);
            stateMap.notifyAll();
        }
    }
